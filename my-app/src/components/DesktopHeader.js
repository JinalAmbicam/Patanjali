...
export default function Header() {
  ...
  const SidebarContent = ({ onClose, ...rest }) => {
    const filteredLinkItems = LinkItems.filter((link) => {
      if (
        ["local", "regional", "zonal", "super", "panIn", "No Role"].includes(Loggeduserrole)
      ) {
        return (
          link.name !== "UserList" &&
          link.name !== "Dashboard" &&
          link.name !== "ManageCamera"
        );
      } else {
        return true;
      }
    ...
    <>
      ...
      >
        <Flex h={16} alignItems="center" justifyContent="space-between">
          <Box>
            <Image style={{ width: "30%" }} src={ptnlogo} alt="Patanjali logo" />
          </Box>
                ...
                <Flex alignItems="center">
                  <Menu>
                    ...
                    <MenuList bg="white" borderColor="gray.200">
                      <MenuItem onClick={handleCallSupport}>Call Support</MenuItem>
                      <MenuItem onClick={handleOpenFAQ}>FAQ</MenuItem>
                      <MenuItem onClick={handleSendEmail}>Email Support</MenuItem>
                      <MenuDivider />
